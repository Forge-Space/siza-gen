...
async function main(): Promise<void> {
  ...
  if (isDryRun) {
    ...
    console.log(
      `Providers found: ${providers.length} (${providers.map((p) => p.type).join(', ') || 'none'})`,
    );
    console.log(`\nDry run — would generate:`);
    console.log(
      `  ${goldenPrompts.length} prompts x ${n} providers = ${goldenPrompts.length * n} generations`,
    );
    console.log(
      `  ${goldenPrompts.length * n * 4} scoring operations`,
    );
    console.log(
      `  ${goldenPrompts.length * 3} enhancement comparisons`,
    );
    process.exit(0);
  }
  ...
  if (providers.length === 0) {
    console.error(
      'No providers available. Set API keys or start Ollama.',
    );
    process.exit(1);
  }

  console.log(
    `Providers: ${providers.map((p) => `${p.type}/${p.model}`).join(', ')}`,
  );
  ...
  const { results, enhancementResults } =
    await harness.runAll(goldenPrompts);
  const durationMs = Date.now() - start;

  const providerMetrics = compareProviders(results);
  const scoringAccuracy = analyzeScoringAccuracy(results);
  const enhancementEffectiveness =
    analyzeEnhancementEffectiveness(enhancementResults);
