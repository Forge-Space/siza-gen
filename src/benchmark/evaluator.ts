...
const PRICING: Record<LLMProviderType, { inputPer1M: number; outputPer1M: number }> = {
...
export function evaluateTraits(code: string, expected: IExpectedTraits): ITraitMatchResult {
  const lower = code.toLowerCase();

  const checks: Record<keyof IExpectedTraits, boolean> = {
    hasAria: !expected.hasAria || /aria-|role=/.test(lower),
    hasResponsive: !expected.hasResponsive || /\b(sm:|md:|lg:|xl:|@media)/.test(code),
    hasDarkMode: !expected.hasDarkMode || /dark:/.test(code),
    hasSemanticHtml: !expected.hasSemanticHtml || /<(nav|main|header|footer|section|article|aside|form)\b/.test(lower),
    hasErrorHandling: !expected.hasErrorHandling || /(error|catch|try|validation|invalid)/i.test(code),
    hasKeyboardNav: !expected.hasKeyboardNav || /(onkeydown|onkeyup|tabindex|focus|keyboard|hotkey)/i.test(code),
  };

  const expectedKeys = (Object.keys(expected) as (keyof IExpectedTraits)[]).filter((k) => expected[k]);
...
export function calculateCost(provider: LLMProviderType, tokensUsed: number, outputRatio = 0.7): number {
...
export function compareProviders(results: IBenchmarkResult[]): IProviderMetrics[] {
  ...
  for (const [provider, items] of grouped) {
    ...
    const scores = successful.map((r) => r.scores.blended ?? r.scores.heuristic);
    ...
    metrics.push({
      ...
      errorRate: items.length > 0 ? items.filter((r) => r.error).length / items.length : 0,
...
export function analyzeScoringAccuracy(results: IBenchmarkResult[]): IScoringAccuracy {
  ...
  return {
    heuristicVsLlmCorrelation: pearsonCorrelation(heuristicScores, llmScores),
    heuristicAvgDelta: avgDelta(heuristicScores, llmScores),
    llmAvgDelta: avgDelta(llmScores, heuristicScores),
    ragImpactAvgDelta: withRag.length > 0 ? avg(withRag.map((r) => (r.scores.rag ?? 0) - r.scores.heuristic)) : 0,
  };
}

export function analyzeEnhancementEffectiveness(enhResults: IEnhancementResult[]): IEnhancementEffectiveness {
  const bySource = (s: string) => enhResults.filter((r) => r.enhancementSource === s);

  const avgDeltaForSource = (s: string) => {
    const items = bySource(s);
    return items.length > 0 ? avg(items.map((r) => r.scoreDelta)) : 0;
  };
  ...
  return {
    ...
    enhancementRate: enhResults.length > 0 ? enhResults.filter((r) => r.scoreDelta > 0).length / enhResults.length : 0,
  };
}
