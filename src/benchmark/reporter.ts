...
export function formatConsole(report: IBenchmarkReport): string {
  ...
  for (const m of report.providerMetrics) {
    lines.push(
      '  ' +
        pad(m.provider, 12) +
        pad(formatMs(m.p50LatencyMs), 10) +
        pad(formatMs(m.p95LatencyMs), 10),
    );
  }
  ...
  lines.push(
    `  Heuristic vs LLM correlation: ${sa.heuristicVsLlmCorrelation.toFixed(3)}`,
  );
  lines.push(
    `  Heuristic avg delta: ${sa.heuristicAvgDelta.toFixed(2)}`,
  );
  lines.push(
    `  RAG impact avg delta: ${sa.ragImpactAvgDelta.toFixed(2)}`,
  );
  ...
  lines.push(
    `  Rules-only avg delta: ${ee.rulesOnlyAvgDelta >= 0 ? '+' : ''}${ee.rulesOnlyAvgDelta.toFixed(2)}`,
  );
  lines.push(
    `  LLM-only avg delta: ${ee.llmOnlyAvgDelta >= 0 ? '+' : ''}${ee.llmOnlyAvgDelta.toFixed(2)}`,
  );
  lines.push(
    `  Combined avg delta: ${ee.combinedAvgDelta >= 0 ? '+' : ''}${ee.combinedAvgDelta.toFixed(2)}`,
  );
  lines.push(
    `  Enhancement positive rate: ${formatPct(ee.enhancementRate)}`,
  );
  ...
  lines.push('  SUMMARY');
  lines.push(
    `  Total generations: ${report.metadata.totalGenerations}`,
  );
  lines.push(
    `  Duration: ${(report.metadata.durationMs / 1000).toFixed(1)}s`,
  );
  const totalCost = report.providerMetrics.reduce(
    (s, m) => s + m.totalCostUsd,
    0,
  );
  ...
  if (report.providerMetrics.length > 0) {
    ...
    lines.push(
      `  Best provider: ${best.provider} (${best.grade}, ${best.avgScore.toFixed(1)}/10)`,
    );
  }
